...
using Stationeers.Addons.Modules.Updater;
using UnityEngine;
...
namespace Stationeers.Addons.Core
{
    ...
    internal class LoaderManager : MonoBehaviour
    {
        ...
        {
            UnityEngine.Debug.Log("ModLoader activated!");
        }
        ...
        private void Awake()
        {
            ...
            if (IsDedicatedServer)
                UnityEngine.Debug.Log("[Stationeers.Addons - DEDICATED SERVER] Stationeers.Addons is running on a dedicated server!");

            // Check if we can debug addons
            IsDebuggingEnabled = File.Exists("addons-debugging.enable");
            if (IsDebuggingEnabled)
                UnityEngine.Debug.Log("[Stationeers.Addons - DEBUG] Stationeers.Addons debugging enabled!");
        ...
        internal IEnumerator Reload()
        {
            if (_isRecompiling)
            {
                UnityEngine.Debug.LogWarning("Already recompiling!");
                yield break;
            }
            
            _isRecompiling = true;
            
            UnityEngine.Debug.Log("Unloading plugins");
            PluginLoader.UnloadAllPlugins();
            
            UnityEngine.Debug.Log("Recompiling plugins");
            yield return PluginCompiler.Load();
            
            UnityEngine.Debug.Log("Reloading plugins");
            yield return PluginLoader.Load();
            
            UnityEngine.Debug.Log("Recompilation done");
        ...
        {
            UnityEngine.Debug.Log("Checking for Stationeers.Addons version...");
            ...
            using (var webRequest = UnityWebRequest.Get(Globals.VersionFile))
            {
                ...
                if (!webRequest.isHttpError && !webRequest.isNetworkError)
                {
                    ...
                    UnityEngine.Debug.Log($"Latest Stationeers.Addons version is {data}. Installed {Globals.Version}");
                    
                    // If the current version is the same as the latest one, just exit the coroutine.
                    if (Globals.Version == data)
                        yield break;
                    
                    UnityEngine.Debug.Log("New version of Stationeers.Addons is available!");
